...
class Healthchecker {
    ...
    private final MicroservicesHub microcloud;
    private final ScheduledExecutorService scheduler;

    public Healthchecker(MicroservicesHub microcloud, ScheduledExecutorService executorService) {
    ...
    private void reporServiceStatus(RemoteMicroservice remote, Boolean faulty) {
        ...
        try {
            send(newHealthMessage(remote, !faulty));
    ...
    private void enquiryApis() {
        ...
        for (RemoteMicroservice remote : microcloud.getMicroServices()) {
            ...
            try {
                send(new MessageBuilder(ENQ, microcloud.getCloud(), remote.getAgent()).make());
            } catch (Exception ex) {
    ...
    private void send(final Message message) throws MsnosException {
        microcloud.getCloud().send(message);
    }
}
