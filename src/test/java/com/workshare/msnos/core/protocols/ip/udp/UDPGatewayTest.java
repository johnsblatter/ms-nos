...
public class UDPGatewayTest {
    ...
    private Message getMessageWithPayload(final BigPayload payload) {
        return new MessageBuilder(MessageBuilder.Mode.RELAXED, Message.Type.PRS, SOMEONE, ME).sequence(12).with(payload).make();
    }
    ...
    private static Message newSampleMessage(final Iden src, final Iden dst) {
        return new MessageBuilder(MessageBuilder.Mode.RELAXED, Message.Type.APP, src, dst).sequence(12).make();
    }
